    ...
    <div draggable
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 9, color: 'var(--text3)', marginTop: 1 }}>
          {!isPitcher(p)
            ? `${(p as any).r}R ${(p as any).d2}2B ${(p as any).hr}HR ${(p as any).rb}RBI .${String((p as any).av.toFixed(3)).slice(1)}`
            : `${(p as any).ip}IP ${(p as any).k}K ${(p as any).er.toFixed(2)}ERA`}
        </div>
      </div>
      <span style={{ fontSize: 10, color: 'var(--text3)', flexShrink: 0 }}>Proj ${p.pr}</span>
